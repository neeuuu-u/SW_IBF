...
- Cleans shapefile municipality names (remove parentheses)
- Normalizes names for matching
- Merges with municipality-level shapefile
- Plots selected columns by name (two color scales)
- Visualizes all municipalities (no-data = dark gray)
- Adds legend patch for "No Data"
...
CSV_ROOT = Path(r"D:\neu\CIM oct28\zones [bst]")
OUTPUT_ROOT = Path(r"D:\neu\CIM oct28\zones_maps_muni_withNaN")
...
def plot_and_save_map(gdf, column, cmap, norm, folder_name, output_file):
    ...
    gdf.plot(ax=ax, color="#444444", edgecolor="black", linewidth=0.2)
    gdf_valid = gdf[~gdf[column].isna()]
    ...
    no_data_patch = Patch(facecolor="#444444", edgecolor="black", label="No Data")
    leg = ax.get_legend()
    if leg is not None:
        ...
        ax.legend([no_data_patch], ["No Data"], loc="lower left", fontsize=8)
...
def main():
    ...
    # --- Clean and normalize municipality names ---
    gdf["Mun_Name_Raw"] = gdf["Mun_Name"]
    gdf["Mun_Name"] = gdf["Mun_Name"].apply(clean_parentheses)
    gdf["prov_norm"] = gdf["Pro_Name"].apply(normalize_text)
    gdf["muni_norm"] = gdf["Mun_Name"].apply(normalize_text)

    # Pre-load color scales
