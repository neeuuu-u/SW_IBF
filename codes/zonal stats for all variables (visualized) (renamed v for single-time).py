# -*- coding: utf-8 -*-
"""
Created on Thu Oct 16 16:54:53 2025
...
- Saves PNG plots for EACH variable and comprehensive CSV zonal stats
"""
import os
import xarray as xr
import numpy as np
...
# CONFIG
# =========================
root_dir = r"D:\neu\GA meeting\timestep\evolution.001-00001"
output_root = r"D:\neu\GA meeting\vizualized (single-time)"
shapefile_path = r"D:\neu\CIM oct28\shp\JC_shp\Municipal_modified.shp"
...
# Variables to process (UPDATED VARIABLE NAMES)
VARIABLES = ['velocity_east', 'velocity_north', 'pressure', 'gust_speed']

# Variable-specific plotting configurations (UPDATED)
VAR_CONFIG = {
    'gust_speed': {
        'label': 'Gust Speed (m/s)',
        'cmap': 'custom',  # Will use wspd_lc()
        'use_custom_cmap': True
    },
    'velocity_east': {
        'label': 'U-component Wind (m/s)',
        'cmap': 'RdBu_r',
        'use_custom_cmap': False
    },
    'velocity_north': {
        'label': 'V-component Wind (m/s)',
        'cmap': 'RdBu_r',
        'use_custom_cmap': False
    },
    'pressure': {
...
# =========================
# Plotting function for ANY variable
# =========================
def plot_variable_with_shapefile(dataset, var_name, output_png, gdf):
    """Plot any NetCDF variable with shapefile overlay"""
    ...
    # Setup colormap
    if config.get('use_custom_cmap') and var_name == 'gust_speed':
        lev, cmap = wspd_lc()
    ...
    cb.set_label(label=config['label'], size=14, labelpad=7)
    ...
    for f in files:
        if f.endswith(".nc"):
            ...
            try:
                ...
                # 1. Create plots for ALL available variables
                print(f"📊 Creating plots for available variables...")
                for var_name in VARIABLES:
                    if var_name in ds:
                        png_path = os.path.join(output_folder, f"{base_name}_{var_name}.png")
                        plot_variable_with_shapefile(ds, var_name, png_path, gdf)
                    else:
                        print(f"⚠️  Variable '{var_name}' not in dataset, skipping plot")
...
print("\n✅ All .nc files processed with multi-variable plots and zonal statistics!")